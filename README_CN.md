...
- **v0.2.10 已发布** ✅ — GUI 坐标映射 + 批量动作 + 敏感区域遮罩 (Issues #38/#39/#40)。(1) **#38 scale_factor 坐标契约** — `GuiResult.scale_factor: f32` (物理 PNG 像素 / 逻辑点; Retina=2.0, 非 Retina=1.0) 桥接两坐标空间: `GuiAction` x/y 输入 + `inspect_tree` AXPosition = 逻辑点, `screenshot_width`/`screenshot_height` = 物理像素。调用方换算 `pixel = point * scale_factor`。由 `CGDisplay::main().bounds().size.width` (逻辑) vs `img.width()` (物理) 计算。4 层 wiring (Rust serde → fe-pyo3 `#[pyo3(get)]` + From → Python Pydantic → fe-ipc done frame serde-flatten)。(2) **#39 gui_action_batch** — 顺序多动作管线, 收集每步 `GuiResult`; 非事务 (单步失败不中止后续, 调用方据每步 `ok` 自决重试/补偿)。`GuiController::gui_action_batch(Vec<GuiAction>) -> Vec<GuiResult>` → fe-core wrapper → fe-ipc `executor.gui_action_batch` arm → fe-pyo3 `gui_action_batch(&PyAny)` (json.dumps → serde) → Python `FusionSandboxExecutor.gui_action_batch(actions: list[dict]) -> list[GuiResult]` (校验 list + 每元素 dict 含 str kind)。(3) **#40 截图敏感区域遮罩** — `GuiAction::Screenshot` 增 `mask_sensitive: bool` (serde 默认 false); 为 true 时 `collect_secure_rects()` 遍历 AX 树 (`MAX_TREE_DEPTH=8`/`MAX_TREE_NODES=500` 上限) 找 `kAXSecureTextFieldSubrole` 元素, 读其 `AXPosition`+`AXSize` (逻辑点矩形), 再 `mask_rgba_inplace()` 经 `scale_factor` 换算物理像素, Y 翻转 (CGContext 原点左下 vs PNG/AX 左上), RGBA 涂黑 (0,0,0,255)。受 `ax_trusted()` 门控 — `mask_sensitive=true` + 未授权 → `warn!` + 跳过遮罩, fail-visible 不崩。截图重构: `cgimage_to_png_b64` 拆为 `cgimage_to_rgba` (CGImage → RGBA 字节缓冲, M-12.3 预乘 α 注释锚定于此) + `rgba_to_png_b64` (RGBA → PNG base64), 使遮罩能在两步间改写字节缓冲。0 新增 unsafe block (复用 fe-gui 既有 3 处 unsafe, 遮罩纯 Rust 字节操作)。+7 Rust 单元测试 + 9 Python 测试。**526 Rust + 222 Python (7 skip TCC) 测试全绿**; clippy `--all-targets -D warnings` 净 (仅上游 block v0.1.6 future-incompat); fmt/ruff 净; maturin 构建。Issues #38/#39/#40 已关闭。
